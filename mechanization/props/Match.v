    ...
    Proof.
      ...
      - (* AtomEscape *)
        ...
        + (* Named backref *)
          ...
          assert (List.Indexing.Nat.indexing (groupSpecifiersThatMatch (zip (AtomEsc (GroupEsc id)) ctx) id) 0 = Success (r, l)) as Eq_indexed. {
            match goal with | [ H: _ = (r, l) :: nil |- _ ] => setoid_rewrite -> H end.
            reflexivity.
          }
          pose proof (EarlyErrors.groupSpecifiersThatMatch_is_filter_map (zip (AtomEsc (GroupEsc id)) ctx) id) as (f & _ & Def_f).
